...
from algs.p2p_bfs import bfs_all
  ...
  def get_dir(self):
    ...
    if self.game.pacman.mode == 'chase':
      ...
      for ghost in self.game.Ghosts:
        temp = bfs_all(self.graph, (self.game.pacman.y, self.game.pacman.x), (ghost.y, ghost.x))
        if temp == None:
          continue
    ...
    if len(self.targets) != 0:
      ...
      for target in self.targets:
        path = bfs_all(self.graph, (self.game.pacman.y, self.game.pacman.x), target)
    ...
    self.queue = bfs_all(self.graph, (self.game.pacman.y, self.game.pacman.x), target)
    return self.queue.pop(0)
